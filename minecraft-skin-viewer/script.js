...
// ============================================================
// 5. Player Skin Loader (via Mojang API + Crafatar)
// ============================================================
...
$(function() {
    ...
    function loadPlayerSkin(username) {
        $statusMsg.text('Fetching UUID for ' + username + '...').css('color', '#888');

        // 使用支持 CORS 的代理 API
        var apiUrl = 'https://corsproxy.io/?' + encodeURIComponent('https://api.mojang.com/users/profiles/minecraft/' + username);
                ...
                if (!data || !data.id) {
                    ...
                    $.getJSON('https://api.minetools.eu/uuid/' + username)
                        .done(function(data2) {
                            ...
                            // minetools 返回的 id 可能不带连字符，需要处理
                            var uuid = data2.id.replace(/(\w{8})(\w{4})(\w{4})(\w{4})(\w{12})/, '$1-$2-$3-$4-$5');
                            applySkin(uuid, data2.name || username);
                        })
                        .fail(function() {
                            $statusMsg.text('❌ Player "' + username + '" does not exist!').css('color', '#e74c3c');
                        });
            ...
            .fail(function() {
                ...
                $.getJSON('https://api.minetools.eu/uuid/' + username)
                    .done(function(data) {
                        ...
                        applySkin(uuid, data.name || username);
                    })
                    .fail(function() {
                        $statusMsg.text('❌ Failed to fetch player data!').css('color', '#e74c3c');
                    });
            });
    }

    function applySkin(uuid, name) {
        $statusMsg.text('✅ Found player: ' + name + ', loading skin...').css('color', '#27ae60');

        // 移除连字符（Crafatar 两种格式都支持，但为了统一）
        ...
        loadSkinFromUrls(skinUrls, 0, name);

        // 显示头像
        var avatarUrl = 'https://crafatar.com/avatars/' + cleanUuid + '?size=48&overlay';
    ...
    function loadSkinFromUrls(urls, index, name) {
        if (index >= urls.length) {
            $statusMsg.text('❌ Failed to load skin from all sources!').css('color', '#e74c3c');
            return;
        }
        ...
        skinImg.onload = function() {
            ...
            $statusMsg.text('✅ Skin loaded successfully!').css('color', '#27ae60');
        };
        skinImg.onerror = function() {
            // 尝试下一个 URL
            loadSkinFromUrls(urls, index + 1, name);
        };
    ...
    function applySkinTexture(skinImg) {
        ...
        if (loader && loader.materials && loader.materials[0]) {
            ...
            $statusMsg.text('⚠️ Model not loaded yet, please refresh and try again.').css('color', '#f39c12');
        }
    }
    ...
    if ($('#playerAvatar').length === 0) {
        $('<img id="playerAvatar" width="48" height="48" style="image-rendering:pixelated; border-radius:4px; display:none;">')
            .insertAfter('#thumb');
    }

    console.log('✅ Skin loader ready! Enter a player name and click "Load Skin".');
});